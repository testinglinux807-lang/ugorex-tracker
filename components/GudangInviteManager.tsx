...
import { createGudangInvite, deleteGudangInvite } from "@/app/actions/users";
...
const STATUS_CLS: Record<GudangInviteItem["status"], string> = {
...
// Kelola link registrasi gudang (tab Akun Gudang di /data, admin): buat
// link sekali-pakai (7 hari), salin, dan cabut.
export function GudangInviteManager({
  invites,
}: {
  invites: GudangInviteItem[];
}) {
  const [state, formAction, pending] = useActionState(
    async (_prev: unknown, fd: FormData) => (await createGudangInvite(fd)) ?? null,
    null,
  );
  const [copiedId, setCopiedId] = useState<string | null>(null);

  async function copy(inv: GudangInviteItem) {
    try {
  ...
  return (
    <div className="space-y-3">
      <form action={formAction} className="flex flex-col gap-2 sm:flex-row">
        <input
          name="note"
          placeholder="Catatan (mis. buat Budi) - opsional"
          className={inputCls}
        />
        <button
          disabled={pending}
        ...
        <ul className="divide-y divide-neutral-100 rounded-lg border border-neutral-200">
          {invites.map((inv) => (
            <li key={inv.id} className="flex items-center gap-2 p-2.5 text-sm">
              ...
              <form
                action={async () => {
                  await deleteGudangInvite(inv.id);
                }}
              >
                <button
                  title="Hapus link"
